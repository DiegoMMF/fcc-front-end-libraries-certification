 ...
 * an input element, button element, and ul element. When 
 * the input element changes, it should trigger a 
 * handleChange() method. Also, the input element should 
 * render the value of input that's in the component's 
 * state. The button element should trigger a submitMessage
...
class DisplayMessages extends React.Component {
    ...
    handleChange() {
        this.setState(event => this.state.input = event)
    }
    submitMessage() {
    ...
    render() {
        return (
            <div>
                ...
                <button type="submit" onSubmit={() => this.submitMessage()}>
                    Submit
                </button>
                <ul>
                    {this.state.messages.map((currentValue, index) => <li key={index}>{currentValue}</li>)}
                </ul>
            </div>
        );
    }
